...
export function StructureGallery({ images }: { images: StructureImage[] }) {
  ...
  return <>
    ...
    {active !== null && <div className="structure-lightbox" role="dialog" aria-modal="true" aria-label={`Galeria ampliada: ${images[active].caption}`} onMouseDown={(event) => { if (event.target === event.currentTarget) setActive(null); }}>
      ...
      <button className="lightbox-nav lightbox-prev" type="button" onClick={showPrevious} aria-label="Imagem anterior"><ArrowLeft strokeWidth={1.3} /></button>
      ...
      <button className="lightbox-nav lightbox-next" type="button" onClick={showNext} aria-label="Próxima imagem"><ArrowRight strokeWidth={1.3} /></button>
    </div>}
  </>;
}
...
export function ReviewsSection({ reviews, sourceUri }: { reviews: Review[]; sourceUri?: string }) {
  ...
  return <section className="section reviews-section" aria-labelledby="reviews-title"><div className="container reviews-heading"><p className="eyebrow">Avaliações</p><div className="reviews-title-block"><h2 id="reviews-title">O que nossos pacientes<br></br>estão falando</h2><p>Experiências reais compartilhadas por quem já passou pela Alpha.</p></div></div><div className="reviews-marquee" aria-label="Avaliações de pacientes no Google"><div className="reviews-track" ref={trackRef}>{[0, 1].map((copy) => <div className="reviews-group" aria-hidden={copy === 1} key={copy}>{reviews.map((review, index) => <ReviewItem review={review} duplicate={copy === 1} onReadMore={setSelectedReview} key={`${copy}-${review.author}-${index}`} />)}</div>)}</div></div><div className="container reviews-attribution">{sourceUri ? <a href={sourceUri} target="_blank" rel="noopener noreferrer" translate="no" className="inline-link-arrow">Ver avaliações no Google <ArrowUpRight className="link-arrow" strokeWidth={1.3} /></a> : <span translate="no">Avaliações reais do Google</span>}</div>{selectedReview && <div className="review-modal" role="dialog" aria-modal="true" aria-labelledby="review-modal-author" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedReview(null); }}><div className="review-modal-card"><button type="button" className="review-modal-close" onClick={() => setSelectedReview(null)} aria-label="Fechar avaliação">×</button><div className="review-author"><span className="review-avatar" aria-hidden="true">{selectedReview.author.trim().charAt(0).toUpperCase()}</span><span><strong id="review-modal-author">{selectedReview.author}</strong><small>{selectedReview.relativeTime}</small></span></div><ReviewStars rating={selectedReview.rating} /><p>“{selectedReview.text}”</p><div className="review-source" translate="no"><GoogleMark /><span>{selectedReview.sourceLabel}</span></div><button type="button" className="review-less" onClick={() => setSelectedReview(null)}>Mostrar menos</button></div></div>}</section>;
